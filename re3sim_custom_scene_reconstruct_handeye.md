...
2. iPhone 主摄可以使用；固定 1× 镜头和拍摄方向，避免中途切镜头、数码变焦或使用 HEIC 作为最终输入
3. 导出为按拍摄顺序命名的小写 `.jpg`、`.jpeg` 或 `.png`
4. 拍 100-150 张作为正式采集；小场景可先用 60-80 张试跑
5. 相邻照片保持约 70-80% 重叠，移动相机位置，不要只站在原地转手机
6. 多拍桌面边缘、背景、目标区域、marker，并补充不同高度和俯视角
7. 光照尽量稳定，不要强反光、曝光跳变
8. 待抓取或之后会移动的物体先移走；固定背景可以保留
9. 不要把机器人手臂频繁拍进去，尤其不能让它在照片间改变姿态
10. marker 最好在多张、分布于不同位置和角度的照片里完整清晰可见
```

不要求机械地完成 360°。如果实验桌后方不可进入，可从左前、正前、
右前拍成 U 形或半环绕，再从可到达的一侧举高手机补充俯视角。背后完全
不可见的表面可能不完整，但只要它不属于任务平台、真实相机常见视角或
必要碰撞区域，就不影响当前目标。

拍摄 reconstruction 照片时，robot base、ChArUco 板、桌面和固定场景之间
的相对关系必须与 Stage 8 使用的物理关系保持一致。hand-eye 数据与场景
照片属于两套独立数据，应放在不同目录；新一轮标定数据也应保存到独立
目录，不能覆盖旧数据后再比较。
